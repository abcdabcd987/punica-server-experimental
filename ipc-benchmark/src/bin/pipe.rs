...
impl OnlineStats {
    ...
    fn std(&self) -> f32 {
        let cnt = self.cnt as f32;
        ((cnt * self.sqr - self.sum.powi(2)) / cnt.powi(2)).sqrt()
    }
}

async fn handle_child(mut child: Child) -> anyhow::Result<()> {
    ...
    loop {
        ...
        stdin.write_u32_le(send_buf.len() as u32).await?;
        stdin.write_all(&send_buf).await?;
        stdin.flush().await?;

        let recv_len = stdout.read_u32_le().await?;
        stdout.read_exact(&mut recv_buf[0..recv_len as usize]).await?;
        let recvtime_ns =
            SystemTime::now().duration_since(basetime).unwrap().as_nanos()
                as u64;
...
async fn pipe_main() -> anyhow::Result<()> {
    let py_path = Path::new(env!("CARGO_MANIFEST_DIR"))
        .join("pipe-subprocess.py")
        .canonicalize()
        .unwrap()
        .display()
        .to_string();
